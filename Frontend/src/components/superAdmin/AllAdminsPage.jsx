...
export default function AllAdminsPage() {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      headers.join(","),
      ...admins.map(a => [a.full_name, a.email, a.user_id, a.department, a.role, a.status].join(","))
    ].join("\n");
    ...
    a.href = url;
    a.download = `admins_list_${new Date().toLocaleDateString()}.csv`;
    a.click();
  };
          ...
          <div className="overflow-x-auto min-h-[300px]">
            {isLoading ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {admins.map((a) => (
                    <tr key={a.user_id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                      ...
                      <td className="py-4 pr-4">
                        <span className={`text-xs font-semibold px-3 py-1 rounded-full capitalize ${
                          a.status === 'approved' ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'
                        }`}>
                          {a.status}
                        </span>
                      </td>
